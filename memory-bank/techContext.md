...
- `web/src/lib/api.ts`: thin fetch wrapper, now also exposing character template/draft/finalize/start and message-regenerate endpoints. The streaming Phase 2 frontend pass adds `reattachStream(requestId, handlers, signal?)` against the new backend `GET /api/requests/{id}/stream` route; it shares the same NDJSON transport via `consumeStream` and just toggles its new `method: "GET" | "POST"` option.
