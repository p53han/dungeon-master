...
- `web/src/lib/npcs.ts` + `npcs.test.ts`: the structural dual of `threads.ts` for the dynamic-NPC inspector surface. `recentlyTouchedNpcIds(state, lookback?)` reads the latest oracle outcome's plural `referenced_npc_ids` (with a fallback to the legacy singular `referenced_npc_id`). `sortNpcsForDisplay(npcs, touched)` returns a stable order — active before retired, recently-touched first within each status group, canonical insertion order preserved within ties, input array not mutated. `NPCsPanel.svelte` consumes both; `Inspector.svelte` owns the derivation. Both helper modules are deliberately pure (no Svelte runes, no DOM) so they unit-test without a component harness.
